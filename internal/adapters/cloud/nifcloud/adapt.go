...
import (
	...
	"github.com/nifcloud/nifcloud-sdk-go/nifcloud/nrn"
...
func NewRootAdapter(ctx context.Context, cfg nifcloud.Config, tracker progress.ServiceTracker) *RootAdapter {
	return &RootAdapter{
		ctx:        ctx,
		tracker:    tracker,
		sessionCfg: cfg,
...
func (a *RootAdapter) SessionConfig() nifcloud.Config {
	return a.sessionCfg
}
...
func (a *RootAdapter) CreateMetadata(resource string) types.Metadata {
	...
	// some services don't require region in the NRN
	// see https://pfs.nifcloud.com/spec/common/nrn.htm
	region := a.region
	...
	return a.CreateMetadataFromNRN((nrn.NRN{
		Partition:  "nifcloud",
		Service:    a.currentService,
		Region:     region,
		NifcloudID: a.accountID,
		Resource:   resource,
	}).String())
}

func (a *RootAdapter) CreateMetadataFromNRN(nrn string) types.Metadata {
	return types.NewRemoteMetadata(nrn)
}
...
func Adapt(ctx context.Context, state *state.State, opt options.Options) error {
	c := &RootAdapter{
		...
		debugWriter:         opt.DebugWriter.Extend("adapt", "nifcloud"),
		concurrencyStrategy: opt.ConcurrencyStrategy,
	}

	c.sessionCfg = config.LoadDefaultConfig()
	...
	computingClient := computing.NewFromConfig(c.sessionCfg)
	_, err := computingClient.DescribeRegions(ctx, nil)
	if err != nil {
		return fmt.Errorf("failed to discover NIFCLOIUD regions: %w", err)
	}

	result, err := computingClient.DescribeUserActivities(ctx, &computing.DescribeUserActivitiesInput{})
	if err != nil {
		return fmt.Errorf("failed to discover NIFCLOIUD caller identity: %w", err)
	}

	if result.UserActivitiesSet == nil || len(result.UserActivitiesSet) == 0 {
		return fmt.Errorf("missing activity log for nifcloud account")
	}

	c.accountID = nifcloud.ToString(result.UserActivitiesSet[0].Operator)
	c.Debug("NIFCLOUD account ID: %s", c.accountID)
